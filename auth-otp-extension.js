...
function codeHash(v){return crypto.createHash('sha256').update(v).digest('hex')}
...
// Keep a list of route overrides. When server.js later calls app.post/app.get,
// the first matching route is replaced by our secure OTP handler.
...
postOverrides.set('/api/auth/verify-email-otp',async(req,res)=>{const id=String(req.body?.challengeId||''),otp=String(req.body?.code||''),c=challenges.get(id);if(!c||c.type!=='register'||c.expiresAt<Date.now())return jsonError(res,401,'Verification code expired.');if(++c.attempts>MAX_OTP_ATTEMPTS||codeHash(otp)!==c.codeHash){if(c.attempts>MAX_OTP_ATTEMPTS)challenges.delete(id);return jsonError(res,401,'Invalid verification code.')}c.user.emailVerified=true;const users=await readUsers();users.push(c.user);await writeUsers(users);challenges.delete(id);const sid=crypto.randomBytes(32).toString('base64url');sessions.set(sid,{userId:c.user.id,expiresAt:Date.now()+7*24*60*60*1000});setCookie(res,sid);res.json({ok:true,user:publicUser(c.user)});});
...
postOverrides.set('/api/auth/verify-login-otp',async(req,res)=>{const id=String(req.body?.challengeId||''),otp=String(req.body?.code||''),c=challenges.get(id);if(!c||c.type!=='login'||c.expiresAt<Date.now())return jsonError(res,401,'Sign-in code expired.');if(++c.attempts>MAX_OTP_ATTEMPTS||codeHash(otp)!==c.codeHash){if(c.attempts>MAX_OTP_ATTEMPTS)challenges.delete(id);return jsonError(res,401,'Invalid sign-in code.')}challenges.delete(id);const sid=crypto.randomBytes(32).toString('base64url');sessions.set(sid,{userId:c.user.id,expiresAt:Date.now()+7*24*60*60*1000});setCookie(res,sid);res.json({ok:true,user:publicUser(c.user)});});
...
postOverrides.set('/api/auth/reset-password',async(req,res)=>{const id=String(req.body?.challengeId||''),otp=String(req.body?.code||''),password=String(req.body?.password||''),c=challenges.get(id);if(!c||c.type!=='reset'||c.expiresAt<Date.now())return jsonError(res,401,'Reset code expired.');if(!validPassword(password))return jsonError(res,400,'Password must be at least 10 characters and contain letters and numbers.');if(++c.attempts>MAX_OTP_ATTEMPTS||codeHash(otp)!==c.codeHash){if(c.attempts>MAX_OTP_ATTEMPTS)challenges.delete(id);return jsonError(res,401,'Invalid reset code.')}const users=await readUsers(),user=users.find(u=>u.id===c.userId);if(!user)return jsonError(res,404,'Account not found.');user.passwordDigest=await hashPassword(password);user.emailVerified=true;await writeUsers(users);challenges.delete(id);res.json({ok:true});});
