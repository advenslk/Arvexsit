...
export const PayHerePaymentPage: React.FC = () => {
  ...
  const amountUsd = Number(params.amount || plan?.monthlyPrice || 0);
  const amountLkr = Math.max(0, amountUsd * 300);
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      attempts += 1;
      try {
        const response = await fetch(`/api/payments/payhere/status?orderId=${encodeURIComponent(paymentOrderId)}`);
  ...
  const startPayment = async () => {
    ...
    try {
      const response = await fetch('/api/payments/payhere/create', {
        ...
        body: JSON.stringify({ orderId: id, item: `${plan.name} - ${params.cycle || 'monthly'}`, customerName, email: customerEmail, phone, address, city, country: 'Sri Lanka', currency: 'LKR', amount: amountLkr }),
      });
      const data = await response.json();
