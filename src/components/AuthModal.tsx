...
import { AlertCircle, ArrowRight, CheckCircle2, Shield, X } from 'lucide-react';
...
export const AuthModal: React.FC = () => {
  ...
  const close=()=>{if(busy)return;setIsAuthModalOpen(false);setErrorMsg('');setSuccessMsg('');setChallengeId('');setOtp('');setMode('auth')};
  const setTab=(tab:'login'|'register'|'admin')=>{setAuthModalTab(tab);setErrorMsg('');setSuccessMsg('');setChallengeId('');setOtp('');setMode('auth')};
  const serverUserLogin=(r:any)=>{if(r?.user)login(r.user.email,r.user.role==='admin'?'admin':'customer',r.user.name,r.user.provider||'email')};

  const submit=async()=>{
    setErrorMsg('');setSuccessMsg('');
    if(challengeId){
      if(!/^\d{6}$/.test(otp)){setErrorMsg('Enter the 6-digit verification code.');return}
      setBusy(true);
      try{
        const endpoint=authModalTab==='admin'?'/api/admin/verify-otp':mode==='forgot'?'/api/auth/reset-password':mode==='verify-register'?'/api/auth/verify-email-otp':'/api/auth/verify-login-otp';
        const body=authModalTab==='admin'?{challengeId,code:otp}:mode==='forgot'?{challengeId,code:otp,password}:{challengeId,code:otp};
    ...
    if(mode==='forgot'){
      ...
      setBusy(true);
      try{
        const r=await fetch('/api/auth/forgot-password',{method:'POST',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:email.trim().toLowerCase()})});
        const j=await r.json().catch(()=>({}));
        if(!r.ok)throw new Error(j.error||'Unable to send reset code.');
        setChallengeId(j.challengeId);setSuccessMsg('A 6-digit reset code was sent to your email.')
      }catch(x){setErrorMsg(x instanceof Error?x.message:'Unable to send reset code.')}finally{setBusy(false)}
      return;
    }
    ...
    try{
      ...
      if(authModalTab==='register'){
        if(j.verificationRequired&&j.challengeId){setChallengeId(j.challengeId);setMode('verify-register');setSuccessMsg(j.message||'A 6-digit verification code was sent to your email.')}else setSuccessMsg(j.message||'Account created.');
        setPassword('');return;
      }
      if(authModalTab==='admin'&&j.requiresTwoFactor){setChallengeId(j.challengeId);setSuccessMsg('Password accepted. A verification code was sent to the administrator email.');return}
      if(j.requiresTwoFactor&&j.challengeId){setChallengeId(j.challengeId);setSuccessMsg(j.message||'A verification code was sent to your email.');return}
      serverUserLogin(j);setSuccessMsg('Signed in successfully.');setTimeout(close,450)
    }catch(x){setErrorMsg(x instanceof Error?x.message:'Unable to reach authentication service.')}finally{setBusy(false)}
  };

  const googleLogin=()=>window.location.assign('/api/auth/google/start');

  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#020208]/85 p-4 backdrop-blur-xl">
      <div className="relative w-full max-w-md overflow-hidden rounded-[30px] border border-white/10 bg-[#0b0d16] shadow-2xl">
        <div className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-purple-600 via-fuchsia-400 to-indigo-500"/>
        <div className="relative p-6 sm:p-8">
          <button onClick={close} className="absolute right-5 top-5 rounded-xl border border-white/5 bg-white/5 p-2 text-slate-400"><X className="h-4 w-4"/></button>
          ...
          <form onSubmit={e=>{e.preventDefault();submit()}} className="space-y-4">
            {challengeId ? <>
              <label className="block"><span className="mb-1.5 block text-xs font-semibold text-slate-300">6-digit verification code</span><input inputMode="numeric" autoComplete="one-time-code" maxLength={6} value={otp} onChange={e=>setOtp(e.target.value.replace(/\D/g,'').slice(0,6))} className="w-full rounded-xl border border-purple-400/20 bg-black/20 py-3 px-4 text-center text-lg font-black tracking-[.45em] text-white outline-none" placeholder="000000"/></label>
              {mode==='forgot'&&<label className="block"><span className="mb-1.5 block text-xs font-semibold text-slate-300">New Password</span><input type="password" value={password} onChange={e=>setPassword(e.target.value)} className="w-full rounded-xl border border-white/10 bg-black/20 py-3 px-4 text-sm text-white outline-none" placeholder="At least 10 characters"/></label>}
            </> : mode==='forgot' ? <label className="block"><span className="mb-1.5 block text-xs font-semibold text-slate-300">Account Email</span><input type="email" value={email} onChange={e=>setEmail(e.target.value)} className="w-full rounded-xl border border-white/10 bg-black/20 py-3 px-4 text-sm text-white outline-none" placeholder="you@example.com"/></label> : <>
              {authModalTab==='register'&&<label className="block"><span className="mb-1.5 block text-xs font-semibold text-slate-300">Full Name</span><input value={name} onChange={e=>setName(e.target.value)} className="w-full rounded-xl border border-white/10 bg-black/20 py-3 px-4 text-sm text-white outline-none" placeholder="Your name"/></label>}
              <label className="block"><span className="mb-1.5 block text-xs font-semibold text-slate-300">Email Address</span><input type="email" value={email} onChange={e=>setEmail(e.target.value)} className="w-full rounded-xl border border-white/10 bg-black/20 py-3 px-4 text-sm text-white outline-none" placeholder="you@example.com"/></label>
              <label className="block"><span className="mb-1.5 block text-xs font-semibold text-slate-300">Password</span><input type="password" value={password} onChange={e=>setPassword(e.target.value)} className="w-full rounded-xl border border-white/10 bg-black/20 py-3 px-4 text-sm text-white outline-none" placeholder={authModalTab==='register'?'At least 10 characters':'Your password'}/></label>
              {authModalTab==='login'&&<button type="button" onClick={()=>{setMode('forgot');setErrorMsg('')}} className="text-xs font-semibold text-purple-300">Forgot password?</button>}
            </>}
          </form>
