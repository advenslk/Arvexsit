...
export const AuthModal: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (challengeId) {
      ...
      setBusy(true);
      try {
        const endpoint = authModalTab === 'admin' ? '/api/admin/verify-otp' : mode === 'forgot' ? '/api/auth/reset-password' : '/api/auth/verify-login-otp';
        ...
        const result = await response.json().catch(() => ({})); if (!response.ok) throw new Error(result.error || 'Verification failed.');
        if (authModalTab === 'admin') { localStorage.setItem('arvex_admin_token', result.token); serverUserLogin(result); setSuccessMsg('Administrator identity verified.'); setTimeout(() => { close(); setIsAdminOpen(true); }, 450); }
        ...
        else { serverUserLogin(result); setSuccessMsg('Signed in successfully.'); setTimeout(close,450); }
    ...
    if (mode === 'forgot') {
      ...
      setBusy(true);
      try { const response = await fetch('/api/auth/forgot-password',{method:'POST',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:email.trim().toLowerCase()})}); const result=await response.json().catch(()=>({})); if(!response.ok)throw new Error(result.error||'Unable to send reset code.'); setChallengeId(result.challengeId); setSuccessMsg('A 6-digit password reset code was sent to your email.'); }
      catch(error){setErrorMsg(error instanceof Error?error.message:'Unable to send reset code.');} finally{setBusy(false);} return;
    }
    ...
    try {
      ...
      if (authModalTab === 'register') { if(result.verificationRequired&&result.challengeId){setChallengeId(result.challengeId);setSuccessMsg(result.message||'A 6-digit verification code was sent to your email.');}else{setSuccessMsg(result.message||'Account created.');} setPassword(''); return; }
      ...
      if(result.requiresTwoFactor&&result.challengeId){setChallengeId(result.challengeId);setSuccessMsg(result.message||'A verification code was sent to your email.');return;}
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#020208]/85 p-4 backdrop-blur-xl">
      <div className="relative w-full max-w-md overflow-hidden rounded-[30px] border border-white/10 bg-[#0b0d16] shadow-2xl shadow-purple-950/30">
        ...
        <div className="relative p-6 sm:p-8">
          ...
          {!challengeId && mode === 'auth' && <div className="mb-5 grid grid-cols-2 rounded-xl border border-white/5 bg-black/20 p-1 text-xs font-bold"><button type="button" onClick={()=>setTab('login')} className={`rounded-lg py-2.5 transition ${authModalTab==='login'?'bg-white text-black':'text-slate-500 hover:text-white'}`}>Sign In</button><button type="button" onClick={()=>setTab('register')} className={`rounded-lg py-2.5 transition ${authModalTab==='register'?'bg-white text-black':'text-slate-500 hover:text-white'}`}>Create Account</button></div>}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {challengeId ? <>
              ...
              {mode === 'forgot' && <label className="block"><span className="mb-1.5 block text-xs font-semibold text-slate-300">New Password</span><div className="relative"><Lock className="absolute left-3 top-3 h-4 w-4 text-slate-600" /><input type="password" value={password} onChange={(e)=>setPassword(e.target.value)} autoComplete="new-password" className="w-full rounded-xl border border-white/10 bg-black/20 py-3 pl-10 pr-4 text-sm text-white outline-none focus:border-purple-400/50" placeholder="At least 10 characters" /></div></label>}
            </> : mode === 'forgot' ? <label className="block"><span className="mb-1.5 block text-xs font-semibold text-slate-300">Account Email</span><div className="relative"><Mail className="absolute left-3 top-3 h-4 w-4 text-slate-600" /><input type="email" value={email} onChange={(e)=>setEmail(e.target.value)} autoComplete="email" className="w-full rounded-xl border border-white/10 bg-black/20 py-3 pl-10 pr-4 text-sm text-white outline-none focus:border-purple-400/50" placeholder="you@example.com" /></div></label> : <>
              ...
              {authModalTab==='login' && <button type="button" onClick={()=>{setMode('forgot');setErrorMsg('');setSuccessMsg('');}} className="text-xs font-semibold text-purple-300 hover:text-purple-200">Forgot password?</button>}
            </>}
            <button disabled={busy} type="submit" className="flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 py-3.5 text-sm font-bold text-white shadow-lg shadow-purple-900/25 transition hover:from-purple-500 hover:to-indigo-500 disabled:opacity-60">{busy?'Securing…':challengeId?(mode==='forgot'?'Reset Password':authModalTab==='admin'?'Verify & Enter Admin':'Verify & Sign In'):mode==='forgot'?'Send Reset Code':authModalTab==='register'?'Create Secure Account':authModalTab==='admin'?'Continue to Verification':'Sign In'}{!busy&&<ArrowRight className="h-4 w-4" />}</button>
          </form>
          {mode === 'forgot' && !challengeId && <button type="button" onClick={()=>{setMode('auth');setErrorMsg('');setSuccessMsg('');setAuthModalTab('login');}} className="mx-auto mt-4 block text-xs font-semibold text-slate-500 hover:text-white">Back to sign in</button>}
          {authModalTab === 'admin' && !challengeId && mode === 'auth' && <p className="mt-5 flex items-center justify-center gap-1.5 text-center text-[10px] leading-5 text-slate-600"><Shield className="h-3 w-3" />Administrator login requires password + one-time email verification.</p>}
          {authModalTab !== 'admin' && !challengeId && mode === 'auth' && <button type="button" onClick={()=>setTab('admin')} className="mx-auto mt-3 block text-[10px] font-semibold text-slate-600 transition hover:text-purple-300">Staff / administrator sign-in</button>}
        </div>
      </div>
    </div>
  );
